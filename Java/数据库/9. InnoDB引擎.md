...
2. MVCC的概念：Multi-Version Concurrency Control，多版本并发控制。用于在多个并发事务同时读写数据库时保持数据的一致性和隔离性。通过维护一个数据行的多个版本来实现的，**快照读为MySQL实现MVCC提供了一个非阻塞读功能**。

3. MVCC的实现原理：隐藏字段 + readView + undo log